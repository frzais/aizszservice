...
def update_envs():
    ...
    from .. import udB
    _keys_to_consider = {"LOG_CHANNEL", "BOT_TOKEN", "BOTMODE", "DUAL_MODE", "language"}
    _envs = dict(os.environ)
    if ".env" in os.listdir("."):
        _env_data = RepositoryEnv(config._find_file(".")).data
        for k, v in _env_data.items():
            _envs.setdefault(k, v)

    for key in _keys_to_consider.union(set(_envs.keys())):
        # Prefer explicit values from env/.env; skip if missing/empty to avoid overwriting DB
        value = _envs.get(key)
        if value is None or str(value).strip() in ("", "None", "null"):
            continue
        # Only set if different to reduce unnecessary writes; never clobber with empties
        if udB.get_key(key) != value:
...
async def autobot():
    ...
    if who.username:
        username = "Aizszv1" + "bot"  # Ganti dengan nama yang diinginkan
    else:
        # Format: who.id + random(1-9) + "bot"
        username = "Aizszv1" + str(random_digit) + "bot"  # Ganti dengan nama yang diinginkan
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 9)
        username = "Aizszv1" + "_bot"  # Ganti dengan nama yang diinginkan
        await ultroid_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await ultroid_bot.get_messages(bf, limit=1))[0].text
...
async def autopilot():
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo, _ = await download_file(
            "https://roastkitchen.website/images/logs.jpg", "channelphoto.jpg"
        )
...
async def customize():
    ...
    try:
        ...
        file = random.choice(
            [
                "https://roastkitchen.website/images/assist.jpg",
                "https://roastkitchen.website/images/assis.jpg",
        ...
        await ultroid_bot.send_message(
            "botfather",
            f"✨ Powerful Aizsz Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @supercesi ✨",
        )
        await asyncio.sleep(2)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying Aizsz Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = None
        BTTS = None  # Button removed
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Aizsz has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(ultroid_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @supercesi\n➖➖➖➖➖➖➖➖➖➖"
